...
namespace {
    ...
    TEST_F(deque_iterator_test, trait_test) {
        static_assert(std::is_same_v<deque_iterator<constructor_stub>::iterator_category, std::random_access_iterator_tag>);
        static_assert(std::is_same_v<deque_iterator<constructor_stub>::value_type, constructor_stub>);
        static_assert(std::is_same_v<deque_iterator<constructor_stub>::reference, constructor_stub&>);
        static_assert(std::is_same_v<deque_iterator<constructor_stub>::pointer, constructor_stub*>);
        static_assert(std::is_same_v<deque_iterator<constructor_stub>::difference_type, std::ptrdiff_t>);
        static_assert(std::random_access_iterator<deque_iterator<constructor_stub> >);
        static_assert(std::random_access_iterator<deque_iterator<constructor_stub, true> >);
    }
    ...
    TEST_F(deque_iterator_test, subscript_test) {
        std::unique_ptr<constructor_stub*[]> cleaner;
        std::unique_ptr<std::unique_ptr<constructor_stub[]>[]> inner_cleaner;
        constructor_stub** data = prepare(cleaner, inner_cleaner);
        data[1][1].id = SPECIAL_VALUE;
        deque_iterator<constructor_stub> it(data, *data);
        it[1 + CHUNK_SIZE<constructor_stub>].id = -SPECIAL_VALUE;
        EXPECT_EQ(it[1 + CHUNK_SIZE<constructor_stub>].id, -SPECIAL_VALUE);
    }

    TEST_F(deque_iterator_test, const_subscript_test) {
        std::unique_ptr<constructor_stub*[]> cleaner;
        std::unique_ptr<std::unique_ptr<constructor_stub[]>[]> inner_cleaner;
        constructor_stub** data = prepare(cleaner, inner_cleaner);
        data[1][1].id = SPECIAL_VALUE;
        data[2][3].id = -SPECIAL_VALUE;
        deque_iterator<constructor_stub> it(data, *data);
        EXPECT_EQ(it[1 + CHUNK_SIZE<constructor_stub>].id, SPECIAL_VALUE);
        EXPECT_EQ(it[3 + 2 * CHUNK_SIZE<constructor_stub>].id, -SPECIAL_VALUE);
        deque_iterator<constructor_stub> it2(data + 2, *(data + 2));
        EXPECT_EQ(it2[-CHUNK_SIZE<constructor_stub> + 1].id, SPECIAL_VALUE);
    }
    ...
    TEST_F(deque_iterator_test, reverse_test) {
        ...
        EXPECT_LT(it2, it);
        EXPECT_GT(it, it2);
        EXPECT_EQ(it - it2, CHUNK_SIZE<constructor_stub> * 4 / 3);
    }
}
